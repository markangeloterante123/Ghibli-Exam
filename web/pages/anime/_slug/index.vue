...
<script>
  ...
  export default {
    components: {
      PageContent: () => import('~/components/anime/PageContent')
    },
    data: () => ({
      loaded: false,
      above_the_fold: null,
    ...
    provide() {
      return {
        above_the_fold: computed(() => this.above_the_fold)
      }
    },
    methods: {
      async getContent() {
        let route = this.$route.params.slug
        await this.getContents.forEach((data, key) => {
          if (route.includes(data.slug)) {
            this.above_the_fold = {
              ...data,
              title: data.title,
              images: data.images
